...
def main():
	...
	status_str1 = subprocess.check_output("ps aux | grep -v grep | grep "+app_pid+" | awk '{print $6}'",shell=True)
	vmrss = int(status_str1) / 1024
	print vmrss
	status_str2 = subprocess.check_output("ps aux | grep -v grep | grep "+app_pid+" | awk '{print $5}'",shell=True)
